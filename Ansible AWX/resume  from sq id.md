giờ t cần 1 script sử dụng kubectl scale 

để scale workload trong k8s 

- mục tiêu: 
	- được sử dụng để sử dụng nhiều workload, trên nhiều namespace khác nhau 
	- input của đầu vào theo định dạng:

```
namespace | <pattern của workload> | <số replicas>
```

	- dựa vào pattern của workload từ input, tìm và lọc ra được workload tương ứng, tự detect là stateful hay deployment
	- Thực hiện scale theo số workload 

```yaml
cron_lines: |
	*/1 * * * * /home/scripts/netserv_master_control.sh
```
...
```yaml
cron_lines: |
	*/1 * * * * /home/scripts/ts_master_control.sh
```
...
curl kbnn-trans-news.sb-backendapi-dev.svc.cluster.local:8080/API_KBNN_TRANS_NEWS/rest/seab/process -v 
